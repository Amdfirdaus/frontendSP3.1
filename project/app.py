...
def get_products():
    ...
    cur.execute("""
        SELECT p.id, p.nama_produk, p.harga, p.gambar,
               p.deskripsi, p.stok, c.nama_kategori
        FROM products p
...
def get_products_by_category(kategori_id):
    cur = mysql.connection.cursor()
    cur.execute("""
        SELECT p.id, p.nama_produk, p.harga, p.gambar, p.deskripsi, p.stok, c.nama_kategori
        FROM products p
        JOIN categories c ON p.kategori_id = c.id
...
def get_product(id):
    cur = mysql.connection.cursor()
    cur.execute("""
    SELECT id, nama_produk, deskripsi, harga, gambar, kategori_id, stok
...
def admin_get_products():
    ...
    cur.execute("""
        SELECT p.id, p.nama_produk,p.deskripsi, p.harga, p.gambar,
               p.kategori_id, p.stok, c.nama_kategori
        FROM products p
        JOIN categories c ON p.kategori_id = c.id
        ORDER BY p.id DESC
    """)
...
def admin_add_product():
    ...
    if not all([nama_produk, deskripsi, harga, kategori_id, stok]):
    ...
    cur = mysql.connection.cursor()
    cur.execute("""
    INSERT INTO products (nama_produk, deskripsi, harga, kategori_id,gambar, stok)
    VALUES (%s, %s, %s, %s, %s, %s)
""", (nama_produk, deskripsi, int(harga), int(kategori_id), filename, int(stok)))
...
def admin_update_product(id):
    ...
    if not all([nama_produk, harga, kategori_id, deskripsi, stok]):
        return jsonify({"error": "Data tidak lengkap"}), 400
