...
const popupPrinter = document.getElementById("popupPrinter");
...
namaBarang.addEventListener("click", () => {
  ...
  if (kategori.value === "printer") popupPrinter.style.display = "flex";
  if (kategori.value === "monitor") popupMonitor.style.display = "flex";
});

// === PILIH ITEM BARANG DARI POPUP ===
document
  .querySelectorAll(
    "#popupLaptop li, #popupAksesoris li, #popupPrinter li, #popupMonitor li"
  )
  .forEach((item) => {
...
document.querySelectorAll(".popup .simpan").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    popupPrinter.style.display = "none";
    popupMonitor.style.display = "none";
  });
});
...
document.getElementById("btnHitung").addEventListener("click", () => {
  ...
  if (kategori.value === "printer") pajakValue = total * 0.16;
  if (kategori.value === "monitor") pajakValue = total * 0.12;
